...
TEST_LABELS = "bdd_dataset/labels/bdd100k_labels_images_val.json"  # Use validation set for evaluation
TEST_IMAGES = "bdd_dataset/100k/val"
...
# Define max samples to run evaluation on (Set `None` for full dataset)
MAX_SAMPLES = 20  # Change this to limit evaluation to the first `N` images
...
with torch.no_grad():
    for images, targets in tqdm.tqdm(test_loader, desc="Evaluating"):
        if MAX_SAMPLES is not None and image_count >= MAX_SAMPLES:
            break  # Stop processing after `MAX_SAMPLES`
        ...
        image_count += len(images)  # Track processed images
...
predicted_labels = sum(predicted_labels, [])  # Flatten lists
true_labels = sum(true_labels, [])  # Flatten lists
...
def visualize_failures(images, predictions, targets, save_dir):
    """
    Saves images showing failed detections.
    """
